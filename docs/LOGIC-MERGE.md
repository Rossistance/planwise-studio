...
| Share sheet | Per-feature share endpoints (audience=team/customer), contacts from meta, `.eml` fallback | Unified sheet: recipients, items w/ internal blocking, when-options | **BUILD UI** over existing routes | Sheet drives the existing endpoints; "hold as a draft"/"with the weekly briefing" map to outbox/briefing attachment. |
