...
| Share sheet | Per-feature share endpoints (audience=team/customer), contacts from meta, `.eml` fallback | Unified sheet: recipients, items w/ internal blocking, when-options | **Deferred — TODO(v2.x)**; per-feature shares shipped | 2.0 ships each feature's own share (look-ahead customer/crew, briefing customer/internal, CO send, record send), every one enforcing the audience rules SERVER-side — the doctrine holds without the sheet. The unified sheet (cross-feature item picking, internal-item blocking against a mixed recipient list, "with the weekly briefing" batching) is the v2.x build; the briefing already batches its look-ahead. |
